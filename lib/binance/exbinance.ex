defmodule Exbinance do
  ...
  Returns `{:ok, [%Exbinance.HistoricalTrade{}]}` or `{:error, reason}`.

  ## Example
  {:ok,
    [%Exbinance.HistoricalTrade{
     id: 192180149,
     is_best_match: true,
     is_buyer_maker: true,
     price: "1.79878000",
  ...
  """
  def get_historical_trades(%Credentials{} = credentials, symbol, limit, from_id)
    ...
    case HTTPClient.unsigned_request_binance(
      ...
      {:ok, data} ->
        {:ok, Enum.map(data, &Exbinance.HistoricalTrade.new(&1))}
  ...
  Returns `{:ok, [%Exbinance.Kline{}]` or `{:error, reason}`
  ...
  def get_klines(symbol, interval, limit \\ 500) when is_binary(symbol) do
    case HTTPClient.get_binance(
      ...
      {:ok, data} ->
        {:ok, Enum.map(data, &Exbinance.Kline.new(&1))}
  ...
  Returns `{:ok, %Exbinance.DataStream{}}` or `{:error, reason}`.

  ## Example
  ```
  {:ok,
    %Exbinance.DataStream{
  ...
  """
  def create_listen_key(%{} = credentials) do
    case HTTPClient.unsigned_request_binance(credentials, "/api/v3/userDataStream", "", :post) do
      {:ok, data} -> {:ok, Exbinance.DataStream.new(data)}
      error -> error
    end
  end
  ...
  """
  def keep_alive_listen_key(%Credentials{} = credentials, key) do
    case HTTPClient.unsigned_request_binance(
           credentials,
  ...
  """
  def close_listen_key(%Credentials{} = credentials, key) do
    case HTTPClient.unsigned_request_binance(
           credentials,
  ...
  Please read https://www.Exbinance.com/restapipub.html#user-content-account-endpoints to understand all the parameters
  """
  def create_order(
    ...
    case HTTPClient.signed_request_binance(credentials, "/api/v3/order", arguments, :post) do
  ...
  Returns `{:ok, %{}}` or `{:error, reason}`
  """
  def order_limit_buy(credentials, symbol, quantity, price, time_in_force \\ "GTC")
  ...
  def order_limit_buy(
    ...
    case find_symbol(symbol) do
      {:ok, binance_symbol} ->
        order_limit_buy(credentials, binance_symbol, quantity, price, time_in_force)
  ...
  def order_limit_buy(%Credentials{} = credentials, symbol, quantity, price, time_in_force)
      when is_binary(symbol)
      when is_number(quantity)
      when is_number(price) do
    create_order(credentials, symbol, "BUY", "LIMIT", quantity, price, time_in_force)
  ...
  Returns `{:ok, %{}}` or `{:error, reason}`
  """
  def order_limit_sell(credentials, symbol, quantity, price, time_in_force \\ "GTC")
  ...
  def order_limit_sell(
    ...
    case find_symbol(symbol) do
      {:ok, binance_symbol} ->
        order_limit_sell(credentials, binance_symbol, quantity, price, time_in_force)
  ...
  def order_limit_sell(credentials, symbol, quantity, price, time_in_force)
      when is_binary(symbol)
      when is_number(quantity)
      when is_number(price) do
    create_order(credentials, symbol, "SELL", "LIMIT", quantity, price, time_in_force)
    |> parse_order_response
  end
  ...
  def order_market_buy(
      ...
      when is_binary(to) do
    case find_symbol(symbol) do
      {:ok, binance_symbol} -> order_market_buy(credentials, binance_symbol, quantity)
      e -> e
    end
  end

  def order_market_buy(%Credentials{} = credentials, symbol, quantity)
      when is_binary(symbol)
      when is_number(quantity) do
    create_order(credentials, symbol, "BUY", "MARKET", quantity)
  end
  ...
  def order_market_sell(
      ...
      when is_binary(to) do
    case find_symbol(symbol) do
      {:ok, binance_symbol} -> order_market_sell(credentials, binance_symbol, quantity)
      e -> e
    end
  end

  def order_market_sell(credentials, symbol, quantity)
      when is_binary(symbol)
      when is_number(quantity) do
    create_order(credentials, symbol, "SELL", "MARKET", quantity)
  end
  ...
  @doc """
  Searches and normalizes the symbol as it is listed on Exbinance.
  ...
  Returns `{:ok, [%Exbinance.Order{}]}` or `{:error, reason}`.
  ...
  ```
  {:ok,
    [%Exbinance.Order{price: "0.1", origQty: "1.0", executedQty: "0.0", ...},
     %Exbinance.Order{...},
     %Exbinance.Order{...},
     %Exbinance.Order{...},
     %Exbinance.Order{...},
  ...
  ```
  """
  def get_open_orders(%Credentials{secret_key: secret_key, api_key: api_key} = _credentials) do
    case HTTPClient.get_binance("/api/v3/openOrders", %{}, secret_key, api_key) do
      {:ok, data} -> {:ok, Enum.map(data, &Exbinance.Order.new(&1))}
      err -> err
    end
  end

  def get_open_orders(
        %Credentials{secret_key: _secret_key, api_key: _api_key} = credentials,
        %Exbinance.TradePair{} = symbol
      ) do
    case find_symbol(symbol) do
      {:ok, binance_symbol} -> get_open_orders(credentials, binance_symbol)
      e -> e
    end
  end
  ...
  ## Example
  ```
  {:ok, %Exbinance.Order{price: "0.1", origQty: "1.0", executedQty: "0.0", ...}}
  ```
  ...
  def get_order(
    ...
    case is_binary(symbol) do
      true ->
        fetch_order(credentials, symbol, timestamp, order_id, orig_client_order_id, recv_window)
  ...
  def fetch_order(
        %Credentials{api_key: api_key, secret_key: secret_key} = _credentials,
        symbol,
        timestamp,
        order_id,
        orig_client_order_id,
    ...
    case HTTPClient.get_binance("/api/v3/order", arguments, secret_key, api_key) do
      {:ok, data} -> {:ok, Exbinance.Order.new(data)}
      err -> err
    end
  end
