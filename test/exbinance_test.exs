defmodule ExbinanceTest do
  ...
  setup_all do
    HTTPoison.start()
    credentials = %Credentials{api_key: "some api_key", secret_key: "some secret_key"}
  ...
  test "get_server_time success return an ok, time tuple" do
    use_cassette "get_server_time_ok" do
      assert Exbinance.get_server_time() == {:ok, 1_616_592_268_319}
    end
  end

  test "get_historical_trades success returns the latest trades", %{credentials: credentials} do
    use_cassette "get_historical_trades_ok" do
      assert {:ok, response} = Exbinance.get_historical_trades(credentials, "XRPUSDT", 1, nil)
      assert [%Exbinance.HistoricalTrade{} | _tail] = response
    end

    use_cassette "get_historical_trades_from_id_ok" do
      assert {:ok, response} = Exbinance.get_historical_trades(credentials, "XRPUSDT", 1, 28457)
      assert [%Exbinance.HistoricalTrade{} | _tail] = response
    end
  end
  ...
  test "get_all_prices returns a list of prices for every symbol" do
    use_cassette "get_all_prices_ok" do
      ...
      assert [%Exbinance.SymbolPrice{price: "0.03040500", symbol: "ETHBTC"} | _tail] =
               symbol_prices
  ...
  describe ".get_ticker" do
    ...
    test "returns an error tuple when the symbol doesn't exist" do
      use_cassette "get_ticker_error" do
        assert Exbinance.get_ticker("IDONTEXIST") == {
                 :error,
  ...
  describe ".get_klines" do
    test "returns the klines for a given symbol and interval" do
      use_cassette "get_klines_ok" do
        assert Exbinance.get_klines("LTCBTC", "1h") ==
                 {
                   :ok,
                   [
                     %Exbinance.Kline{
    ...
    test "returns error with invalid interval" do
      use_cassette "get_klines_interval_err" do
        assert Exbinance.get_klines("LTCBTC", "1") ==
  ...
  describe ".get_depth" do
    ...
    test "returns an error tuple when the symbol doesn't exist" do
      use_cassette "get_depth_error" do
        assert Exbinance.get_depth("IDONTEXIST", 1000) == {
                 :error,
                 %{"code" => -1121, "msg" => "Invalid symbol."}
               }
      end
  ...
  describe ".order_limit_buy" do
    test "creates an order with a duration of good til cancel by default", %{
      credentials: credentials
    } do
      use_cassette "order_limit_buy_good_til_cancel_default_duration_success" do
        assert {:ok, %Exbinance.OrderResponse{} = response} =
                 Exbinance.order_limit_buy(credentials, "LTCBTC", 0.1, 0.01)
    ...
    test "creates an order with a duration of good til cancel by default(string quantity and price)",
    ...
    test "can create an order with a fill or kill duration", %{credentials: credentials} do
      use_cassette "order_limit_buy_fill_or_kill_success" do
        assert {:ok, %Exbinance.OrderResponse{} = response} =
                 Exbinance.order_limit_buy(credentials, "LTCBTC", 0.1, 0.01, "FOK")
    ...
    test "can create an order with am immediate or cancel duration", %{credentials: credentials} do
      use_cassette "order_limit_buy_immediate_or_cancel_success" do
        assert {:ok, %Exbinance.OrderResponse{} = response} =
                 Exbinance.order_limit_buy(credentials, "LTCBTC", 0.1, 0.01, "IOC")
    ...
    test "returns an insufficient balance error tuple", %{credentials: credentials} do
      use_cassette "order_limit_buy_error_insufficient_balance" do
        assert {:error, reason} =
                 Exbinance.order_limit_buy(credentials, "LTCBTC", 10_000, 0.001, "FOK")
    ...
    test "properly formats orders with very low prices", %{credentials: credentials} do
      ...
      use_cassette "order_limit_buy_very_low_price", custom_matchers: [matches_price] do
        assert {:ok, %Exbinance.OrderResponse{} = response} =
                 Exbinance.order_limit_buy(credentials, "DOGEBTC", 100, 0.000001)
  ...
  describe ".order_limit_sell" do
    test "creates an order with a duration of good til cancel by default", %{
      ...
      use_cassette "order_limit_sell_good_til_cancel_default_duration_success" do
        assert {:ok, %Exbinance.OrderResponse{} = response} =
                 Exbinance.order_limit_sell(credentials, "BTCUSDT", 0.001, 50_000)
    ...
    test "can create an order with a fill or kill duration", %{credentials: credentials} do
      use_cassette "order_limit_sell_fill_or_kill_success" do
        assert {:ok, %Exbinance.OrderResponse{} = response} =
                 Exbinance.order_limit_sell(credentials, "BTCUSDT", 0.001, 50_000, "FOK")
    ...
    test "can create an order with am immediate or cancel duration", %{credentials: credentials} do
      use_cassette "order_limit_sell_immediate_or_cancel_success" do
        assert {:ok, %Exbinance.OrderResponse{} = response} =
                 Exbinance.order_limit_sell(credentials, "BTCUSDT", 0.001, 50_000, "IOC")
  ...
  describe ".get_open_orders" do
    test "when called without symbol returns all open orders for all symbols", %{
      credentials: credentials
    } do
      use_cassette "get_open_orders_without_symbol_success" do
        assert {:ok, [%Exbinance.Order{} = order_1, %Exbinance.Order{} = order_2]} =
                 Exbinance.get_open_orders(credentials)
    ...
    test "when called with symbol returns all open orders for that symbols(string)", %{
      credentials: credentials
    } do
      use_cassette "get_open_orders_with_symbol_string_success" do
        assert {:ok, [%Exbinance.Order{} = result]} =
                 Exbinance.get_open_orders(credentials, "WABIBTC")
    ...
    test "when called with symbol returns all open orders for that symbols(TradePair struct)", %{
      credentials: credentials
    } do
      use_cassette "get_open_orders_with_trade_pair_struct_string_success" do
        assert {:ok, [%Exbinance.Order{} = result]} =
                 Exbinance.get_open_orders(credentials, %Exbinance.TradePair{
                   :from => "WABI",
                   :to => "BTC"
                 })
  ...
  describe ".cancel_order" do
    test "when called with symbol(struct), orderId and timestamp cancels order", %{
      credentials: credentials
    } do
      use_cassette "cancel_order_by_struct_symbol_orderId_and_timestamp_success" do
        assert {:ok, %Exbinance.Order{} = order} =
                 Exbinance.cancel_order(
                   credentials,
                   %Exbinance.TradePair{:from => "XRP", :to => "USDT"},
                   1_564_000_518_279,
                   212_213_771
                 )
    ...
    test "when called with symbol(string), orderId and timestamp cancels order", %{
      credentials: credentials
    } do
    ...
    test "when called with symbol(string), clientOrderId and timestamp cancels order", %{
      credentials: credentials
    } do
